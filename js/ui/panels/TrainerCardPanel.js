 ...
 *
 * Edit mode keeps EXACT same layout — labels become inline inputs
...
// Input occupies the SAME visual space as the value text it replaces.
// Transparent background, thin bottom border, same font metrics.

// Shared input classes — matches view-mode value styling exactly
const _editInputCls = 'bg-transparent border-0 border-b-2 border-blue-400 dark:border-blue-500 text-xl font-black text-gray-900 dark:text-white tracking-tight uppercase text-right outline-none focus:border-blue-600 dark:focus:border-blue-400 w-full max-w-[160px] p-0 m-0 transition-colors';
...
function _renderTrainerEditFields(form, trainer, isYellow, adapter, validator) {
    ...
    if (gen >= 2) {
        ...
        const genderOptions = validator.getOptions('trainer', 'gender') || ['Male', 'Female'];
        html += _editStatBlock('Gender',
            `<select id="edit-gender" class="bg-transparent border-0 border-b-2 border-blue-400 dark:border-blue-500 text-xl font-black text-gray-900 dark:text-white tracking-tight uppercase text-right outline-none focus:border-blue-600 dark:focus:border-blue-400 p-0 m-0 cursor-pointer w-full max-w-[160px] transition-colors">
                ${genderOptions.map(g => `<option value="${g}" ${g === gender ? 'selected' : ''}>${g}</option>`).join('')}
            </select>`,
...
function _renderBadgesSection(badges, trainer, isEditing, form, adapter, hasJohtoBadges, hasKantoBadges) {
    ...
    if (hasJohtoBadges) {
        ...
        const johtoEarned = johtoBadges.filter((_, relIdx) => {
            const i = johtoStartIdx + relIdx;
            if (isEditing) return form.badges?.[i] ?? false;
            return ((trainer.badges >> i) & 1) === 1;
        }).length;
        html += `
            <div class="flex items-center gap-1.5 mb-3">
                ${_trophySVG}
                <h4 class="text-xs font-black text-gray-400 uppercase tracking-widest">Johto Badges</h4>
                <span class="ml-auto text-xs font-bold text-amber-500">${johtoEarned}/${johtoBadges.length}</span>
            </div>
            <div class="bg-white/50 dark:bg-gray-800/80 p-4 rounded-[1.5rem] border-2 border-dashed border-amber-200 dark:border-amber-900/40 grid grid-cols-4 gap-3 shadow-inner mb-3">
    ...
    if (hasKantoBadges) {
        ...
        const kantoEarned = kantoBadgesList.filter((_, relIdx) => {
            const i = kantoStartIdx + relIdx;
            if (adapter && adapter.generationId >= 2 && i >= 8) {
                if (isEditing) return form.badges?.[i] ?? false;
                const kantoByte = (trainer.badges >> 8) & 0xFF;
                return ((kantoByte >> (i - 8)) & 1) === 1;
            }
            if (isEditing) return form.badges?.[i] ?? false;
            return ((trainer.badges >> i) & 1) === 1;
        }).length;
        html += `
            <div class="flex items-center gap-1.5 mb-3">
                ...
                <span class="ml-auto text-xs font-bold text-blue-500">${kantoEarned}/${kantoBadgesList.length}</span>
            </div>
            <div class="bg-white/50 dark:bg-gray-800/80 p-4 rounded-[1.5rem] border-2 border-dashed border-blue-200 dark:border-blue-900/40 grid grid-cols-4 gap-3 shadow-inner">
    ...
    if (!hasJohtoBadges && !hasKantoBadges && badges.length > 0) {
        html += `
            ...
            <div class="bg-white/50 dark:bg-gray-800/80 p-4 rounded-[1.5rem] border-2 border-dashed border-gray-200 dark:border-gray-700 grid grid-cols-4 gap-3 shadow-inner">
...
const _actionBtnBase = 'flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all duration-200 border';
        ...
        <div class="${bodyBg} rounded-[2.5rem] shadow-2xl border-4 border-gray-100 dark:border-gray-800 overflow-hidden relative flex flex-col transition-all duration-300">
            ...
            <div class="px-5 pt-14 pb-6 space-y-3 flex-grow ${bodyBg}">
                ...
                <div class="flex justify-end items-center gap-2 mb-2">
                    ...
                    ` : `
                        <button id="trainer-save-btn" class="${_actionBtnBase} bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 border-green-200 dark:border-green-800/40 hover:bg-green-100 dark:hover:bg-green-900/30 hover:shadow-md">
                        ...
                        <button id="trainer-cancel-btn" class="${_actionBtnBase} bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border-red-200 dark:border-red-800/40 hover:bg-red-100 dark:hover:bg-red-900/30 hover:shadow-md">
                ...
                <div class="space-y-2">
                    ${isEditing
                        ? _renderTrainerEditFields(form, trainer, isYellow, adapter, validator)
...
export function bindEvents(container, eventBus, theme, appState, localState, updateFn) {
    ...
    if (editBtn) {
        editBtn.addEventListener('click', () => {
            ...
            // Build initial form from current trainer data
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', () => {
            ...
            const updates = {
                name: nameVal.value,
                rivalName: rivalVal.value,
                id: idVal.value,
                money: moneyVal.value,
                playTime: playTimeVal.value,
                badges: badgesByte,
            };

            // Optional fields
            if (coinsEl) {
                const coinsVal = validator.validateField('trainer', 'coins', coinsEl.value ? Number(coinsEl.value) : form.coins);
                updates.coins = coinsVal.value;
            }
            if (pikachuEl) {
                const friendVal = validator.validateField('trainer', 'pikachuFriendship', Number(pikachuEl.value));
                updates.pikachuFriendship = friendVal.value;
            }
            if (genderEl) {
                const genderVal = validator.validateField('trainer', 'gender', genderEl.value);
                updates.gender = genderVal.value;
            }
            ...
            // Show toast with save status
            const errors = [nameVal, rivalVal, idVal, moneyVal, playTimeVal].filter(r => !r.valid);
            if (errors.length > 0) {
                appState.showToast(`Trainer card saved. Some values were clamped to valid ranges.`);
            }
