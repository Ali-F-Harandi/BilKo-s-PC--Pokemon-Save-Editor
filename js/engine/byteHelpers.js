/**
 * byteHelpers.js — Binary Utility Functions
 *
 * Ported from lib/utils/byteHelpers.ts
 * BCD parsing, big-endian reads, bit operations, status decoding.
 */

// Helper: Parse BCD (Binary Coded Decimal) used for Money
export function parseBCD(data, start, length) {
  let result = 0;
...
// Helper: Write BCD
export function setBCD(view, offset, value, length) {
  let temp = value;
...
export function countSetBits(data, start, length) {
  let count = 0;
  for (let i = 0; i < length; i++) {
...
export function setUInt16BigEndian(view, offset, value) {
...
export function setUInt24BigEndian(view, offset, value) {
...
export class BitUtils {
  ...
  static getRange(value, start, length) {
    return (value >> start) & ((1 << length) - 1);
  }
}

// Helper: Decode Status Byte
export function decodeStatus(byte) {
  if (byte === 0) return "OK";
...
export function getAsciiString(view, start, length) {
  let str = '';
  for (let i = 0; i < length; i++) {
